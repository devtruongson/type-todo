...
import { Notify, Task, keys } from "../constants";
import TaskModel from "../models/task.model";
import { messageNotify } from "../constants/notify";
...
class TaskView {
    ...
    init = (): void => {
        ...
        const clearCompletedButton = querySelector(
            ".clear-completed"
        ) as HTMLElement;
    ...
    renderTasks = (tasks: Task[]): void => {
        ...
        const incompleteTasksCount = this.taskModel
            .getTasks("getall")
            .filter((item: Task) => !item.isCompleted).length;
        const itemsText = incompleteTasksCount === 1 ? "item" : "items";
        this.totalElement.innerHTML =
            incompleteTasksCount > 1
                ? `${incompleteTasksCount} ${itemsText} left`
                : `${incompleteTasksCount} item left`;
        this.taskList.innerHTML = tasks
            .map(
                ({
                    ...
                    `<li data-id="${id}"  data-checked="${
                        isCompleted ? "true" : "false"
                    }" class="content-data">
                    <i class="fa fa-circle-o task-icon"></i>
                    <i class="fa fa-check-circle-o checkmark" style="display: ${
        ...
        closeIcons.forEach((icon: Element) => {
            icon.addEventListener(
                "click",
                this.handleTaskClose as EventListener
            );
        });

        const contentDataItems =
            this.taskList.querySelectorAll(".content-data");
        contentDataItems.forEach((item: Element) => {
            const contentText = item.querySelector(
                ".task-content"
            ) as HTMLElement;
            const editInput = item.querySelector(
                ".edit-input"
            ) as HTMLInputElement;
    ...
    handleContentDataClick = (event: MouseEvent) => {
        ...
        if (clickedElement.classList.contains("task-icon")) {
            const taskDataId = parseInt(
                clickedElement.parentElement?.dataset.id || "NaN"
            );
            ...
            if (taskDataId || (taskDataId === 0 && currentStatus)) {
                ...
                const checkmark = clickedElement.parentElement?.querySelector(
                    ".checkmark"
                ) as HTMLElement;
                const taskContentElement =
                    clickedElement.parentElement?.querySelector(
                        ".task-content"
                    ) as HTMLElement;
    ...
    handleTaskClose = (event: MouseEvent): void => {
        const taskId = (event.currentTarget as HTMLElement).getAttribute(
            "data-task-id"
        );
    ...
    handleTaskDoubleClick = (taskId: number): void => {
        const content = this.taskList.querySelector(
            `.editable-content[data-task-id="${taskId}"]`
        );
        const contentText = content?.querySelector(
            ".task-content"
        ) as HTMLElement;
        const editInput = content?.querySelector(
            ".input-edit"
        ) as HTMLInputElement;
    ...
    updateClearCompletedButtonVisibility(): void {
        ...
        if (clearCompletedButton) {
            clearCompletedButton.style.display = hasCompletedTasks
                ? "block"
                : "none";
        }
    }
    ...
    setTaskEditedHandler = (
        callback: (taskId: number, newContent: string) => void
    ): void => {
